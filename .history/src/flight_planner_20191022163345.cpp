...
void FlightPlanner::step()
{
  ...
  float xCmd, yCmd, zCmd;
   ...
   getLocalPositionOffset(localOffset, current_gps_location, start_gps_location);

  double xOffsetRemaining = target_position_vector[0] - localOffset.x;
  double yOffsetRemaining =target_position_vector[1] - localOffset.y;
  double zOffsetRemaining = target_position_vector[2] - localOffset.z;

   info_counter++;
  if(info_counter > 25)
  {
    info_counter = 0;
    ROS_INFO("-----x=%f, y=%f, z=%f", localOffset.x,localOffset.y, localOffset.z);
    ROS_INFO("+++++dx=%f, dy=%f, dz=%f", xOffsetRemaining,yOffsetRemaining, zOffsetRemaining);
  }
